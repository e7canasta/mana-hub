...
class AlarmProfileApplicationService(
    ...
    fun updateResidentProfile(residentId: String, request: UpdateAlarmProfileRequest): AlarmProfileResponse {
        ...
        val (riskLevel, templateId) = resolveEffectiveFields(request, current)
        val newProfile = createNewProfile(rid, request, current, riskLevel, templateId)

        alarmProfileRepository.save(newProfile)
        eventPublisher.publishAndClear(newProfile)

        migrateOverrides(request, current, newProfile)
        publishChangedEvent(residentId, newProfile, riskLevel, templateId)

        return getResidentProfile(residentId)!!
    }

    private fun resolveEffectiveFields(
        request: UpdateAlarmProfileRequest,
        current: AlarmProfileVersion?,
    ): Pair<RiskLevel, TemplateId?> {
        ...
        return riskLevel to templateId
    }
    ...
    ) = AlarmProfileVersion.create(rid, request.updatedBy).update(
    ...
    private fun migrateOverrides(
        request: UpdateAlarmProfileRequest,
        current: AlarmProfileVersion?,
        newProfile: AlarmProfileVersion,
    ) {
    ...
    private fun publishChangedEvent(
        residentId: String,
        newProfile: AlarmProfileVersion,
        riskLevel: RiskLevel,
        templateId: TemplateId?,
